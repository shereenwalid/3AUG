...
- Other documents (quotes, PO, order forms, emails, HLD, etc.) may ONLY be
  used to fill HEADER fields (everything except "line_items") that the tech
  spec does not cover. They are validation material, not an item source.
- If the tech spec and another document conflict on a header field, prefer
  the tech spec value and append a short note about the conflict in the
  description field.
- If NO tech spec document is present, you may still fill header fields from
  the other documents, and must state "Tech spec document not found - filled
  from other sources" at the start of the description field - but "line_items"
  MUST be returned as an empty array (see the LINE ITEMS - TECH SPEC ONLY rule
  below). Never substitute another document as the item source.

LINE ITEMS - TECH SPEC ONLY (HARD RULE):
"line_items" (the sales order rows) MUST be built EXCLUSIVELY from the TECH
SPEC document. This is not a preference, it is a hard constraint:
- NEVER add, infer, or supplement a line item using the PO, customer quote,
  vendor quote, circuit order form, HLD, email, or any document other than
  the tech spec - even if that other document clearly lists a product,
  quantity, or price the tech spec omits.
- Every other document exists ONLY to support the validation checks (already
  performed in the previous step). They must never be used as a source of
  order rows here.
- If the tech spec is missing, unreadable, or contains no product/service
  lines, return "line_items": [] (an empty array) and explain why in the
  description field. Do NOT pull items from another document to compensate.
- The only line items ever added from outside the tech spec's own product
  list are the PREDEFINED PART CODES below, which come from a fixed
  configuration table (not from another document) and apply to circuit
  orders only.
...
line_items:
    Fill EXCLUSIVELY from the TECH SPEC document - it is the ONLY source of
    what is being ordered. Never take, add, or backfill a line item from any
    other document (PO, quotes, order form, HLD, email, etc.) - those are for
    validation only, per the LINE ITEMS - TECH SPEC ONLY rule above. If the
    tech spec has no product/service lines, return an empty "line_items"
    array rather than sourcing rows elsewhere. One entry per product/service
    line from the tech spec, each with:
    - quantity
    ...
    The Product Code Navigator ran in parallel and resolved the FINAL item
    codes for the products in the tech spec. Its result is available as
    {resolved_product_codes?} - a list of entries each with "resolved"
    (formatted "code - name/desc") and "confidence" (High/Medium/Low/None).
