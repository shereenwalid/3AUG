...
You are scoped to the TECH SPEC document(s) ONLY. You are given ONLY the
tech-spec-only document content below - no PO, no signed contract, no
customer/vendor quotes, no order form, no HLD, no other opportunity
document. Those other files are for the validation agent only and are
deliberately NOT provided to you. The FINAL SALES ORDER ROWS (line_items)
and every other field on this template must come from the tech spec alone.
...
Validation report (runs in parallel with you, so it may not be ready yet -
treat this as optional context only, never required):
{validation_report?}
If {validation_report?} is empty or missing, ignore it and proceed with the
extraction from the tech spec alone - do not wait for it and do not treat
its absence as an error.

If {tech_spec_data?} is empty or missing, do NOT fall back to any other
document. Instead fill every field 'Not Specified', leave line_items empty,
and state "Tech spec document not found - cannot extract" at the start of
the description field.

Steps:
0. ALWAYS perform the extraction regardless of the validation report's
   presence or content (FAILED checks, NOT READY verdict, or missing
   entirely). Validation never blocks extraction - just fill what you can
   from the tech spec.
1. FIRST THING: read the tech spec content above IN FULL - all of it, no
   matter how long. Never skip or skim any part of it.
2. Treat the tech spec content as your ONLY knowledge base for every field,
   including all line_items.
...
4. If no value can be determined confidently from the tech spec, write:
   'Not Specified'.
...
customer_reference:
    The PO number if it appears in the tech spec; if no PO number is present
    in the tech spec, write 'Not Specified' (do not look in other documents -
    they are out of scope for you).
description:
...
line_items:
    ...
    RESOLVED PRODUCT CODES (from the Product Code Navigator):
    The Product Code Navigator runs IN PARALLEL with you (same tech spec
    input, no dependency between you) and resolves the FINAL item codes for
    the products in the tech spec. Its result MAY OR MAY NOT be ready yet -
    do not wait for it. If available it is in {resolved_product_codes?} - a
    list of entries each with "resolved" (formatted "code - name/desc") and
    "confidence" (High/Medium/Low/None).
    For each product line, MATCH it to the navigator entry (by the product
    stock code or description) and then:
    - set "item" to the navigator's "resolved" value exactly (the
      "code - name/desc" string). Do NOT invent or alter it.
    - set "confidence" to the navigator's "confidence" for that product.
    If a product has no matching navigator entry, keep the tech-spec code as
    "item" and set "confidence" to "None". Do not invent codes.

    EVERY line item MUST include a "confidence" field with one of exactly:
    "High", "Medium", "Low", or "None". Never omit it and never leave it blank.
    If {resolved_product_codes?} is empty or missing, set every row's
    "confidence" to "None".

    PREDEFINED PART CODES (CIRCUIT ORDERS ONLY):
    If this is a CIRCUIT order, identify the order type and look it up in the
    PREDEFINED PART CODES table below. EVERY part code listed for that order
    type MUST appear as a line item - IN ADDITION to the products extracted
    from the tech spec.
    - If the tech spec already contains that code, keep the tech spec's row
      (its quantity / price / location).
    - If a predefined code is MISSING, ADD it as a new line item: put the code
      in both "item" and "sku", quantity 1, location = the order's site, and
      price / charge_type / recurring_period / fulfilment = "Not Specified"
      (a human completes these), and note in the item that it was added from
      the predefined part-code table.
    - Match the order type flexibly (case-insensitive, ignore minor spacing).
      If the order type does not match any row, add nothing extra.
    - For an either/or entry written "CODE_A / CODE_B", pick the one matching
      the product (e.g. IPVPN vs DIA); if unclear, add both and flag for review.
...

{
  ...
  "device_services": "...",
  "line_items": [
    {"quantity": "...", "item": "<code - name/desc>", "sku": "...",
     "location": "...", "price": "...", "charge_type": "...",
     "recurring_period": "...", "fulfilment": "...", "confidence": "High|Medium|Low|None"}
  ]
}
...
# ── Inject the predefined part-codes table (single source of truth: config) ──
def _format_part_codes_table() -> str:
    try:
        from .config import PART_CODES_BY_ORDER_TYPE
    except Exception:  # noqa: BLE001
        return "PREDEFINED PART CODES table unavailable."
    lines = ["PREDEFINED PART CODES (order type -> required part codes):"]
    for order_type, codes in PART_CODES_BY_ORDER_TYPE.items():
        lines.append(f"  {order_type}: {', '.join(codes)}")
    return "\n".join(lines)


# Fill the {part_codes_table} placeholder once, at import time.
EXTRACTION_PROMPT = EXTRACTION_PROMPT.replace(
    "{part_codes_table}", _format_part_codes_table()
)


# ─────────────────────────────────────────────────────────────────────
# Final join / UI-mapping node.
# Consolidates the three parallel branches (validation, product code
# navigator, BSP extraction) into one payload. It is a pure PASSTHROUGH -
# it must NEVER regenerate, reinterpret, or "improve" any value; the
# per-field / per-check UI mapping is done deterministically in Python
# (agent_api.py: _shape_result). This node exists so the graph has exactly
# one terminal node, and so a single state key holds everything the UI
# needs in one place.
# ─────────────────────────────────────────────────────────────────────
FINAL_OUTPUT_PROMPT = """
You are the Final Output Mapper. You do NOT validate, extract, or resolve
anything yourself - you only assemble the results the other three agents
already produced into one JSON object, UNCHANGED.

Inputs (each may be missing if that branch hasn't produced output - treat
missing as null, never invent a replacement value):

opportunity_id: {opportunity_id?}
matched_folder: {opportunity_folder?}
template_date: {template_date?}

validation_report (from qto_validation_agent):
{validation_report?}

bsp_order (from bsp_extraction_agent):
{bsp_order?}

resolved_product_codes (from product_code_agent):
{resolved_product_codes?}

TASK:
Return ONLY this JSON object, no markdown, no commentary:
{
  "opportunity_id": "<opportunity_id above, or '' if missing>",
  "matched_folder": "<matched_folder above, or '' if missing>",
  "template_date": "<template_date above, or '' if missing>",
  "validation_report": <the validation_report value above EXACTLY as given,
                         parsed as JSON if it is a JSON string, or null if
                         missing - do not alter any field or value inside it>,
  "bsp_order": <the bsp_order value above EXACTLY as given, parsed as JSON
                if it is a JSON string, or null if missing - do not alter
                any field, line item, or value inside it>,
  "resolved_product_codes": <the resolved_product_codes value above EXACTLY
                              as given, or null if missing>
}

Rules:
- Copy values verbatim. Never summarize, reformat, re-derive, or "fix" a
  value from any of the three upstream agents.
- Never fabricate a value for a field that is missing above - use null / ""
  as specified.
